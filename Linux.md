...
- rm 删除文件
- rm -r 删除目录
- rm -f 强制删除，避免删除多层文件时的多次确认

------

**touch、cat、tac、more、less、head、tail、grep** <!--创建、查看文件内容、分页显示文件内容-->

- cat -n 显示文件内容时加上行号，适合内容较少的小文件
- tac 将文件内容倒序显示
- more  进入more查看文件状态**空格**翻页，**Enter**换行，**q或Q**退出
- less  与more相似，但可以向上翻页和搜索，**page up**向上翻页，**上箭头**向上翻一行。**/** + **关键词**搜索，,而more不可以
- head -n 8 文件名 表示查看文件前8行，-n指定行数
- tail -n 10 文件名 查看文件末尾10行 
- tail -f 动态显示文件末尾内容
- grep -i 查找内容 文件路径, 文件内匹配查找
...
- chmod 777 文件名 改变文件的权限使之成为rwxrwxrwx
